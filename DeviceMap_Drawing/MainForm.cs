...
namespace DeviceMap_Drawing
{
    public partial class MainForm : Form
    {
        ...
        {
            Invalidate();           // OnPaint 강제 호출 (guide 위치 저장)
            Application.DoEvents(); // guidePosition 확정 후 컨트롤 생성
            
            CalculateChipCenters(); // chipCenters 먼저 계산                        
            AddGuideControls();     // Label + Numeric 생성
        }
        ...
        private void AddGuideControls()
        {
            ...
            {
                { "PKG_X_Pitch",     Tuple.Create(-70, -65, -70, -35, 80) },
                { "PKG_Y_Pitch",     Tuple.Create(-60, -30, -60,   0, 80) },
                { "PKG_X_Size",      Tuple.Create(-70,  15, -70,  40, 80) },
                { "PKG_Y_Size",      Tuple.Create(-60, -30, -60,   0, 80) },
                { "Wafer_Real_Size", Tuple.Create(-120, -30, -120, 0, 80) },
                { "Safety_Distance", Tuple.Create(10, -20, 10, 10, 80) }
            };
            ...
            void AddPair(string key, string labelText, int decimalPlaces)
            {
                ...
                nud.Minimum = 0;
                nud.Maximum = 1000;
                nud.Value = (decimalPlaces == 3) ? 0.500M : 10.0M;
                nud.Width = nudW;
